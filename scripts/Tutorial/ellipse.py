...
extra1 = (
    Gradius * d3d.Point3D(Gdir.vector) * math.cos(math.pi / 2)
    + Sradius * d3d.Point3D(Sdir.vector) * math.sin(math.pi / 2)
    + center
)

segbh = d3d.LineSegment3D(
    pointellipse, pointellipse + d3d.Point3D([i * h for i in normal.vector])
)  # point on the ellipse not on the center
