...
c1 = primitives2D.ClosedRoundedLineSegments2D(
    points1, {2: 0.005, 3: 0.005, 4: 0.005, 8: 0.005, 9: 0.005, 10: 0.005, 14: 0.005}
)
...
profile1 = primitives3D.Cylinder(
    position=d3d.Point3D(-0.0075, 0.14, 0.25), axis=d3d.X3D, radius=0.02, length=0.015, name="profile1"
)

profile2 = primitives3D.Cylinder(
    position=d3d.Point3D(-0.0075, 0.19, 0.25), axis=d3d.X3D, radius=0.02, length=0.015, name="profile2"
)

profile3 = primitives3D.Cylinder(
    position=d3d.Point3D(-0.0075, 0.15, 0.08), axis=d3d.X3D, radius=0.08, length=0.015, name="profile3"
)


y = d3d.X3D.random_unit_normal_vector()
z = d3d.X3D.cross(y)
profile4 = primitives3D.RevolvedProfile(
    d3d.Z3D * 0.08 - 0.015 * d3d.Y3D, d3d.X3D, z, c5, d3d.Z3D * 0.08 - 0.015 * d3d.Y3D, d3d.X3D
)
profile5 = primitives3D.RevolvedProfile(
    d3d.Z3D * 0.22 + 0.035 * d3d.Y3D, d3d.X3D, z, c5, d3d.Z3D * 0.22 + 0.035 * d3d.Y3D, d3d.X3D
)
...
sweep1 = primitives3D.Sweep(contour2d=c, wire3d=rl1, name="pipe1")
