...
poly1_vol1 = d3dw.ClosedPolygon3D(
    [
        d3d.Point3D(-0.1, -0.05, 0),
...
#
faces = [d3df.Triangle3D(*points) for points in poly1_vol1.sewing(poly2_vol1, d3d.X3D, d3d.Y3D)] + [
    d3df.Triangle3D(*points) for points in poly2_vol1.sewing(poly3_vol1, d3d.X3D, d3d.Y3D)
]
...
# shell1.babylonjs()
#
poly1_vol2 = d3dw.ClosedPolygon3D(
    [
        d3d.Point3D(-0.1, -0.1, -0.2),
...
faces_2 = (
    [d3df.Triangle3D(*points) for points in poly1_vol2.sewing(poly2_vol2, d3d.X3D, d3d.Z3D)]
    + [d3df.Triangle3D(*points) for points in poly2_vol2.sewing(poly3_vol2, d3d.X3D, d3d.Z3D)]
    + [d3df.Triangle3D(*points) for points in poly3_vol2.sewing(poly4_vol2, d3d.X3D, d3d.Z3D)]
    + [d3df.Triangle3D(*points) for points in poly4_vol2.sewing(poly5_vol2, d3d.X3D, d3d.Z3D)]
)
