...
class Component:
    def __init__(self, center, compo_side, vector1, vector2, height, plane):
        ...
        self.primitives = [
            d3d.LineSegment2D(self.points[0], self.points[1]),
            d3d.LineSegment2D(self.points[1], self.points[2]),
            d3d.LineSegment2D(self.points[2], self.points[3]),
            d3d.LineSegment2D(self.points[3], self.points[0]),
        ]
    ...
    def compo_solid(self):
        ...
        return primitives3D.ExtrudedProfile(
            self.plane.origin,
            ...
            color=(0.44313725, 0.27058824, 0.12156863),
        )
...
for step in range(0, nb_step):
    ...
    contour = primitives2D.ClosedRoundedLineSegments2D(poly.points, radius, adapt_radius=True)
    offset_radius = []
    ...
    sides = primitives3D.ExtrudedProfile(
        origin, x, y, outer_contour, [inner_contour], height_bot_belt * extrusion_vector, alpha=alpha, name="sides"
    )

    bottom = primitives3D.ExtrudedProfile(
        origin, x, y, outer_contour, [], -thickness_min * extrusion_vector, alpha=alpha, name="bottom"
    )
    ...
    for enum, polyfloor in enumerate(list_polyfloor[1:]):
        ...
        contour_floor = primitives2D.ClosedRoundedLineSegments2D(polyfloor.points, radius_floor, adapt_radius=True)
        ...
        if enum == 0:
            belt_top = primitives3D.ExtrudedProfile(
                origin + basis_plane.normal * height_origin,
                x,
                y,
                belt_outer_contour,
                [inner_contour_floor] + screw_holes,
                height_belt * basis_plane.normal,
                alpha=alpha,
                name="belt_top",
            )
        ...
        sides_floor = primitives3D.ExtrudedProfile(
            origin + extrusion_vector * height_origin,
            x,
            y,
            outer_contour_floor,
            [inner_contour_floor],
            (list_height_polyfloor[enum + 1] - height_origin) * basis_plane.normal,
            alpha=alpha,
            name="sides_floor",
        )
        ...
        if with_borders:
            ...
            arc = d3d.Arc2D(
                d3d.Point2D((0, 0)),
                d3d.Point2D(((-1 + math.sqrt(2) / 2) * r, r * math.sqrt(2) / 2)),
                d3d.Point2D((-r, r)),
            )
            ...
            wire_sweep = d3d.Wire3D(
                [
                    p.To3D(origin + extrusion_vector * list_height_polyfloor[enum + 1], x, y)
        ...
        if enum == len(list_polyfloor) - 2:
            if with_borders:
                ...
                arc_top = d3d.Arc2D(
                    d3d.Point2D((0, 0)),
                    d3d.Point2D(((-1 + math.sqrt(2) / 2) * r_top, r_top * math.sqrt(2) / 2)),
                    d3d.Point2D((-r_top, r_top)),
                )
                contour_top = d3d.Contour2D([arc_top])
                contour_sweep_top = outer_contour_floor.Offset(r_top)

                new_wire_sweep = d3d.Wire3D(
                    [
                        p.To3D(origin + extrusion_vector * (list_height_polyfloor[enum + 1]), x, y)
                        for p in contour_sweep_top.primitives
                    ]
                )
                ...
                top = primitives3D.ExtrudedProfile(
                    ...
                    contour_sweep_top,
                    [],
                    thickness_min * basis_plane.normal,
                    alpha=alpha,
                    name="top",
                )
            else:
    ...
    if with_borders:
        for k, contour_floor in enumerate(list_contour_floor):
            if k < len(list_contour_floor) - 1:
                rooftop = primitives3D.ExtrudedProfile(
                    origin + extrusion_vector * list_height_polyfloor[k + 1],
                    x,
                    y,
                    contour_floor,
                    [list_inner[k + 1]],
                    r * basis_plane.normal,
                    alpha=alpha,
                    name="rooftop",
                )
            else:
...
steps = [
    [d3d.OXYZ] * nb_components + [away_frame] * (nb_primitives - nb_components),
    ...
    [d3d.OXYZ] * (nb_components + 3 + len(primitives_floor[0]))
    + [away_frame] * (nb_primitives - nb_components - 3 - len(primitives_floor[0])),
    [d3d.OXYZ] * (nb_components + 3 + len(primitives_floor[0]) + len(primitives_floor[1]))
    + [away_frame] * (nb_primitives - nb_components - 3 - len(primitives_floor[0]) - len(primitives_floor[1])),
