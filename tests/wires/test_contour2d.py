...
class TestContour2D(unittest.TestCase):
    ...
    contour1 = wires.Contour2D(
        [edges.FullArc2D(circle=curves.Circle2D(design3d.OXY, 0.029999999), start_end=design3d.Point2D(0.029999999, 0))]
    )
    ...
    point_to_extract_with = [
        (point1_, point2_),
        (design3d.Point2D(0.15, -0.05), design3d.Point2D(0.15, 0.05)),
        (design3d.Point2D(-0.15, 0.15), point2_),
        (design3d.Point2D(-0.15, 0.15), point1_),
    ]
    ...
    unordered_contour2_unittest = wires.Contour2D(
        [line_segment2, bspline.reverse(), arc.reverse(), line_segment1, line_segment3, line_segment4]
    )

    invalid_unordered_contour2_unittest = wires.Contour2D(
        [
            line_segment2,
            bspline.reverse(),
            arc.reverse(),
    ...
    def test_point_inside(self):
        ...
        contour, point = SerializableObject.from_json(
            os.path.join(folder, "test_contour_point_belongs.json")
        ).primitives
    ...
    def test_order_contour(self):
        ...
        for previous_primitive, primitive in zip(
            ordered_contour.primitives, ordered_contour.primitives[1:] + [ordered_contour.primitives[0]]
        ):
    ...
    def test_cut_by_wire(self):
        ...
        contour1 = wires.ClosedPolygon2D(
            [design3d.Point2D(0, 0), design3d.Point2D(1, 0), design3d.Point2D(1, 1), design3d.Point2D(0, 1)]
        )
    ...
    def test_edge_crossings(self):
        points = [
            design3d.Point2D(-0.3, -0.2),
            design3d.Point2D(0.3, -0.2),
            design3d.Point2D(0.2, 0.2),
            design3d.Point2D(0, 0.35),
            design3d.Point2D(-0.2, 0.2),
        ]
        contour = wires.ClosedPolygon2D(points)
        lineseg = edges.LineSegment2D(design3d.Point2D(-0.3, -0.3), design3d.Point2D(0.3, 0.3))
        lineseg1 = edges.LineSegment2D(design3d.Point2D(-0.3, -0.3), design3d.Point2D(0.2, 0.2))
    ...
    def test_crossings(self):
        ...
        expected_crossings = [
            design3d.Point2D(0.003455042474764269, 0.010365521626940934),
        ...
        points = [
            ...
            design3d.Point2D(0.3, -0.2),
            design3d.Point2D(0.2, 0.2),
            design3d.Point2D(0, 0.3),
            design3d.Point2D(-0.2, 0.2),
        ]
        contour = design3d.wires.ClosedPolygon2D(points)
        primitives = [
            design3d.edges.LineSegment2D(design3d.Point2D(-0.35, -0.1), design3d.Point2D(-0.1, 0)),
        ...
        wire3 = design3d.wires.Wire2D(
            primitives[:-1] + [design3d.edges.LineSegment2D(design3d.Point2D(0.2, 0.2), design3d.Point2D(0.1, 0.0))]
        )
    ...
    def test_contours_from_edges(self):
        source_folder = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "test_contour2d_contours_from_edges_json_files"
        )
        expected_contour_areas = [
        ...
        contour_areas = []
        contour_lengths = []
        for filename in ["1_test_contours_from_edges.json", "2_test_contours_from_edges2.json"]:
    ...
    def test_divide(self):
        ...
        expected_contour_areas = [
            0.0006684483866419249,
            0.002005345159820638,
            0.002005345159845676,
    ...
    def test_merge_not_adjacent_contour(self):
        ...
        contour2 = design3d.wires.Contour2D.from_points(
            [
                design3d.Point2D(-0.014284827066811355, 0.00644881122080076),
                design3d.Point2D(-0.008, 0.0060),
    ...
    def test_cut_by_line(self):
        contour, line = wires.Contour2D.from_json(os.path.join(folder, "test_contour2d_cut_by_line.json")).primitives
