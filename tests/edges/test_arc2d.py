...
class TestArc2D(unittest.TestCase):
    ...
    arc2d = Arc2D(circle2d, design3d.Point2D(-1, 0), design3d.Point2D(1, 0))
    ...
    def test_arc_angles(self):
        ...
        start = design3d.Point2D(-21.897737241977318, 47.18790043228133)
        end = design3d.Point2D(0.5512862323650758, 50.732948667420146)
        ...
        arc1 = Arc2D(self.circle2d, design3d.Point2D(1, 0), design3d.Point2D(0, 1))
        arc2 = Arc2D(self.circle2d, design3d.Point2D(0, 1), design3d.Point2D(-1, 0))
        arc3 = Arc2D(self.circle2d, design3d.Point2D(-1, 0), design3d.Point2D(0, -1))
        arc4 = Arc2D(self.circle2d, design3d.Point2D(0, -1), design3d.Point2D(1, 0))
        circle_reversed = self.circle2d.reverse()
        arc5 = Arc2D(circle_reversed, design3d.Point2D(1, 0), design3d.Point2D(0, -1))
        arc6 = Arc2D(circle_reversed, design3d.Point2D(0, -1), design3d.Point2D(-1, 0))
    ...
    def test_split(self):
        ...
        arc_split3 = self.arc2d.split(design3d.Point2D(0, -1))
        self.assertTrue(arc_split3[0].start.is_close(self.arc2d.start))
        self.assertTrue(arc_split3[0].end.is_close(design3d.Point2D(0, -1)))
        self.assertTrue(arc_split3[1].start.is_close(design3d.Point2D(0, -1)))
    ...
    def test_point_belongs(self):
        ...
        point = design3d.Point2D(0.01330629098214331, 0.0032923224261096617)
        self.assertTrue(arc.point_belongs(point))

        arc = Arc2D.from_json(os.path.join(folder, "arc2d_point_belongs.json"))
        point = design3d.Point2D(0.0007151488183559929, 0.007258543823331798)
        self.assertTrue(arc.point_belongs(point))
    ...
    def test_point_distance(self):
        ...
        point4 = design3d.Point2D(0, -1)
        self.assertEqual(arc.point_distance(point4), math.sqrt(2))

    def test_rotation(self):
        rotated_arc2d = self.arc4.rotation(design3d.O2D, math.pi / 4)
        self.assertTrue(rotated_arc2d.start.is_close(design3d.Point2D(0.9999999999999999, -1.1102230246251565e-16)))
        self.assertTrue(rotated_arc2d.end.is_close(design3d.Point2D(1.1102230246251565e-16, 0.9999999999999999)))

        # Test on non trigo Arc2D
        rotated_arc2d = self.arc9.rotation(design3d.O2D, math.pi / 4)
        rotated_arc2d = rotated_arc2d.rotation(design3d.O2D, -math.pi / 4)
        self.assertTrue(rotated_arc2d.is_close(self.arc9))

        # Test on trigo Arc2D
        rotated_arc2d = self.arc2.rotation(design3d.O2D, math.pi / 2)
        rotated_arc2d = rotated_arc2d.rotation(design3d.O2D, -math.pi / 2)
        self.assertTrue(rotated_arc2d.is_close(self.arc2))

        arc2d = Arc2D.from_json(os.path.join(folder, "arc2d_rotation_test.json"))
        rotated_arc2d = arc2d.rotation(design3d.Point2D(0.5, 0.5), math.pi / 1.5)
        self.assertEqual(arc2d.frame.u, rotated_arc2d.frame.u)
        self.assertEqual(arc2d.frame.v, rotated_arc2d.frame.v)
        self.assertTrue(rotated_arc2d.center.is_close(design3d.Point2D(-0.17631397208144173, 0.5714101615137755)))
        self.assertTrue(rotated_arc2d.start.is_close(design3d.Point2D(-0.4330127018922192, 1.1160254037844388)))
        self.assertTrue(rotated_arc2d.end.is_close(design3d.Point2D(-0.3660254037844387, 2.220446049250313e-16)))

    def test_translation(self):
        translated_arc2d = self.arc4.translation(design3d.Vector2D(1, 1))
        self.assertTrue(translated_arc2d.start.is_close(design3d.Point2D(1.7071067811865475, 0.29289321881345254)))
        self.assertTrue(translated_arc2d.end.is_close(design3d.Point2D(1.7071067811865475, 1.7071067811865475)))

        # Test on non trigo Arc2D
        translated_arc2d = self.arc9.translation(design3d.Vector2D(1, 1))
        translated_arc2d = translated_arc2d.translation(design3d.Vector2D(-1, -1))
        self.assertTrue(translated_arc2d.is_close(self.arc9))

        # Test on trigo Arc2D
        translated_arc2d = self.arc2.translation(design3d.Vector2D(1, 1))
        translated_arc2d = translated_arc2d.translation(design3d.Vector2D(-1, -1))
    ...
    def test_center_of_mass(self):
        ...
        self.assertTrue(center_of_mass.is_close(design3d.Point2D(0.6002108774380707, -6.663679357622141e-17)))
    ...
    def test_straight_line_center_of_mass(self):
        ...
        self.assertTrue(center_of_mass.is_close(design3d.Point2D(0.8258716790243482, -9.169017534386686e-17)))

    def test_straight_line_point_belongs(self):
        straight_line_point_belongs = self.arc4.straight_line_point_belongs(design3d.Point2D(0.8258716790243482, -0.2))
        self.assertTrue(straight_line_point_belongs)
        self.assertFalse(self.arc4.straight_line_point_belongs(design3d.Point2D(0.8258716790243482, 0.8)))

    def test_to_3d(self):
        vector1 = design3d.Vector3D(1, 1, 1)
        vector1 = vector1.unit_vector()
        vector2 = vector1.deterministic_unit_normal_vector()
        arc3d = self.arc4.to_3d(design3d.O3D, vector1, vector2)
        self.assertTrue(arc3d.start.is_close(design3d.Point3D(-0.169101978726, 0.696923425059, 0.696923425059)))
        self.assertTrue(arc3d.end.is_close(design3d.Point3D(0.985598559653, 0.119573155869, 0.119573155869)))
    ...
    def test_infinite_primitive(self):
        ...
        self.assertTrue(infinite_primitive.start.is_close(design3d.Point2D(1.0606601717798214, -1.0606601717798214)))
        self.assertTrue(infinite_primitive.end.is_close(design3d.Point2D(1.0606601717798214, 1.0606601717798214)))

    def test_from_3_points(self):
        points_ = [design3d.Point2D(-1, 0), design3d.Point2D(0, -1), design3d.Point2D(1, 0)]
        ...
        self.assertEqual(arc_from_3_points.rotation(design3d.O2D, 10).rotation(design3d.O2D, -10), arc_from_3_points)
        self.assertEqual(arc_from_3_points2.rotation(design3d.O2D, 10).rotation(design3d.O2D, -10), arc_from_3_points2)
        self.assertEqual(arc_from_3_points.translation(design3d.X2D).translation(-design3d.X2D), arc_from_3_points)
