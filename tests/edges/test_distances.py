...
arc_ellipse3d = d3de.ArcEllipse3D(
    ellipse3d,
...
class TestEdgesDistances(unittest.TestCase):
    ...
    def test_edges_distances_3d(self):
        ...
        distances = []
        for edge1, edge2 in product([bspline_curve3d(), linesegment3d, arc3d, arc_ellipse3d], repeat=2):
            if edge1 == edge2:
                edge2 = edge2.frame_mapping(
                    design3d.Frame3D(design3d.Point3D(2, 5, -3), vector1, vector2, vector3), "new"
                )
            dist, min_dist_point_arc, min_dist_point_lineseg = edge1.minimum_distance(edge2, True)
            distances.append(dist)
        for distance, expected_distance in zip(distances, expected_distances):
