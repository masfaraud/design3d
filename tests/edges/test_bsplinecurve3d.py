...
class TestBSplineCurve3D(unittest.TestCase):
    ...
    def test_bounding_box(self):
        bspline = d3de.BSplineCurve3D.from_points_interpolation(
            [
                ...
                design3d.Point3D(0.0, 0.0, 1.0),
                design3d.Point3D(-0.30901699437494734, -0.30901699437494734, 0.9510565162951533),
                design3d.Point3D(-0.8090169943749473, -0.8090169943749473, 0.587785252292473),
                design3d.Point3D(-1.0, -1.0, 0.0),
            ],
            2,
            centripetal=True,
    ...
    def test_trim(self):
        ...
        bspline, point1, point2 = SerializableObject.from_json(
            os.path.join(folder, "test_bspline_trim271123.json")
        ).primitives
        ...
        for pt1, pt2 in [
            (bspline.start, point1),
            (point1, bspline.start),
            (point2, bspline.end),
            (point1, point2),
            (point2, point1),
        ]:
        ...
        bspline = d3de.BSplineCurve3D.from_json(os.path.join(folder, "bsplinecurve_split_bug_2.json"))
    ...
    def test_from_step(self):
        obj_list = design3d.core.VolumeModel.from_json(
            os.path.join(folder, "periodic_bsplinecurve_from_step_test_object_dict.json")
        ).primitives
        ...
        self.assertTrue(
            bsplinecurve.point_at_abscissa(0.5 * bsplinecurve.length()).is_close(
                design3d.Point3D(0.04915593260514362, -0.04264529220680001, 0.14332598788877735)
            )
        )
    ...
    def test_bspline_linesegment_intersections(self):
        ...
        self.assertTrue(intersections[0].is_close(design3d.Point3D(-3.0, 1.0222488954206392, 0.5746069850600913)))
    ...
    def test_point_at_abscissa(self):
        ...
        self.assertTrue(
            bspline.point_at_abscissa(0.5 * bspline.length()).is_close(
    ...
    def test_local_discretization(self):
        edge, start, end = SerializableObject.from_json(
            os.path.join(folder, "test_bspline_local_discretizations.json")
        ).primitives
        expected_points = [
            design3d.Point3D(0.40000000000000013, 0.3055497472688364, -0.0577802904293785),
            design3d.Point3D(0.39999999999999997, 0.3054558765193882, -0.05729389615629579),
            design3d.Point3D(0.4, 0.3053650847805137, -0.056818942485284164),
            design3d.Point3D(0.4000000000000001, 0.30527736613101497, -0.05635539982690565),
            design3d.Point3D(0.4, 0.3051925979747842, -0.055902690345743224),
            design3d.Point3D(0.4000000000000001, 0.3051105616115585, -0.055459784623149766),
            design3d.Point3D(0.4, 0.3050309628076735, -0.055025298319996994),
            design3d.Point3D(0.39999999999999997, 0.3049534523668191, -0.054597588839424546),
            design3d.Point3D(0.39999999999999997, 0.30487764670079315, -0.05417485198958898),
            design3d.Point3D(0.4000000000000001, 0.3048031484002564, -0.05375521864641269),
        ]
    ...
    def test_move_frame_along(self):
        ...
        control_points = [
            ...
            design3d.Point3D(0.3, 0.2, 0.1),
            design3d.Point3D(0.5, -0.1, 0.4),
            design3d.Point3D(0.5, -0.4, 0.0),
            design3d.Point3D(-0.1, -0.2, -0.3),
            design3d.Point3D(-0.3, 0.4, 0.1),
        ]
        knots = [0.0, 1.0]
        knot_multiplicities = [6, 6]
        weights = None  # [1, 2, 1, 2, 1, 2]
        bspline_curve3d = d3de.BSplineCurve3D(
            degree=degree,
        ...
        frame = design3d.Frame3D(
            origin=design3d.Point3D(-1.0, 0.0, 0.0),
            u=design3d.Vector3D(0.16926811079722504, -0.8799271690658463, -0.44393297220065076),
            v=design3d.Vector3D(1.3877787807814457e-17, 0.4504326973383234, -0.8928103858986645),
            w=design3d.Vector3D(0.9855700414821559, 0.15112432732120784, 0.076243891719756),
        )
        self.assertEqual(
            bspline_curve3d.move_frame_along(frame),
            design3d.Frame3D(
                origin=design3d.Point3D(-0.3, 0.4, 0.1),
                u=design3d.Vector3D(0.9632101019095523, 0.22376214189648538, 0.14885161548766362),
                v=design3d.Vector3D(2.7755575615628914e-17, 0.553867484333956, -0.8326048341185484),
                w=design3d.Vector3D(-0.26874951084493176, 0.8019733871217126, 0.5334907560296971),
            ),
        )
