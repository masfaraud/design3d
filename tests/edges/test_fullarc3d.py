...
class TestFullArc3D(unittest.TestCase):
    ...
    circle = curves.Circle3D(
        design3d.Frame3D(center, design3d.X3D, design3d.Y3D, design3d.Z3D), center.point_distance(start_end)
    )
    fullarc3d = edges.FullArc3D(circle, start_end)

    def test_linesegment_intersections(self):
        line3d = edges.LineSegment3D(design3d.Point3D(-0.2, -0.2, -0.25), design3d.Point3D(0.2, 0.2, -0.25))
        line3d_ = edges.LineSegment3D(design3d.Point3D(-0.15, 0, 0), design3d.Point3D(-0.15, 0.0, -0.35))
        ...
        self.assertTrue(
            linesegment_intesections1[0].is_close(design3d.Point3D(0.10606601717798213, 0.10606601717798214, -0.25))
        )
        self.assertTrue(
            linesegment_intesections1[1].is_close(design3d.Point3D(-0.10606601717798213, -0.10606601717798213, -0.25))
        )
