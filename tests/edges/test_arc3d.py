...
class TestArc3D(unittest.TestCase):
    ...
    arc3d_2 = edges.Arc3D(curves.Circle3D(design3d.OXYZ, 1), design3d.Point3D(-1, 0, 0), design3d.Point3D(1, 0, 0))
    ...
    def test_from_3_points(self):
        points = [
            design3d.Point3D(-0.2672612419124244, -0.5345224838248488, -0.8017837257372732),
            design3d.Point3D(0.9636241116594316, -0.14824986333222026, -0.22237479499833038),
            design3d.Point3D(0.2672612419124244, 0.5345224838248488, 0.8017837257372732),
        ]
    ...
    def test_discretization_points(self):
        expected_points = [
            ...
            design3d.Point3D(0.9996507193171729, -0.01868742048356531, -0.01868742048356531),
            design3d.Point3D(0.5334021057067797, -0.5981146184585249, -0.5981146184585249),
            design3d.Point3D(-0.40696605032273603, -0.6459019406553569, -0.6459019406553569),
            design3d.Point3D(-0.9855985560899309, -0.11957317055561367, -0.11957317055561367),
        ]
    ...
    def test_abscissa(self):
        ...
        point = design3d.Point3D(0.6002208894332702, -0.637646466964, 0.006570128575852758)
    ...
    def test_direction_vector(self):
        ...
        self.assertTrue(
            direction_vector.is_close(design3d.Vector3D(-0.5773502691896258, -0.5773502691896258, -0.5773502691896258))
        )

    def test_normal_vector(self):
        normal_vector = self.arc3d.normal_vector(self.arc3d.abscissa(self.list_points[2]))
        self.assertTrue(
            normal_vector.is_close(design3d.Vector3D(-0.8164965809277261, 0.408248290463863, 0.408248290463863))
        )

    def test_rotation(self):
        rotated_arc3d = self.arc3d.rotation(design3d.O3D, self.arc3d.circle.frame.v, math.pi / 2)
        self.assertTrue(
            rotated_arc3d.start.is_close(
                design3d.Point3D(-1.9127081796092275e-16, -0.7071067811865475, 0.7071067811865477)
            )
        )
        self.assertTrue(
            rotated_arc3d.end.is_close(design3d.Point3D(-0.5773502691896253, 0.7886751345948129, -0.21132486540518736))
        )

    def test_translation(self):
        translated_arc3d = self.arc3d.translation(self.arc3d.circle.frame.w)
        self.assertTrue(
            translated_arc3d.start.is_close(
                design3d.Point3D(0.5773502691896258, 1.2844570503761734, -0.12975651199692173)
            )
        )
        self.assertTrue(
            translated_arc3d.end.is_close(
                design3d.Point3D(-0.9855985596534886, 0.5875336253174973, -0.8266799370555978)
            )
        )

    def test_frame_mapping(self):
        frame_mapped_arc3d = self.arc3d.frame_mapping(self.arc3d.circle.frame, "new")
        self.assertTrue(frame_mapped_arc3d.start.is_close(design3d.Point3D(0.9999999999999998, 0.0, 0.0)))
        self.assertTrue(
            frame_mapped_arc3d.end.is_close(design3d.Point3D(-0.7071067811865477, -0.7071067811865472, 0.0))
        )

    def test_to_2d(self):
        arc2d = self.arc3d.to_2d(self.arc3d.circle.center, self.arc3d.circle.frame.u, self.arc3d.circle.frame.v)
        self.assertTrue(arc2d.start.is_close(design3d.Point2D(1.0, 0.0)))
        self.assertTrue(arc2d.end.is_close(design3d.Point2D(-0.7071067811865477, -0.7071067811865471)))
    ...
    def test_minimum_distance_points_arc(self):
        ...
        self.assertTrue(
            ...
            or minimum_distance_points_arc[0].is_close(
                design3d.Point3D(-0.9855985596534887, -0.11957315586905082, -0.11957315586905082)
            )
        )
        self.assertTrue(
            minimum_distance_points_arc[1].is_close(
                design3d.Point3D(0.5773502532353942, 1.2844570583532893, -0.12975650401980598)
            )
            or minimum_distance_points_arc[1].is_close(
                design3d.Point3D(-0.8164965809277258, 1.1153550716504106, -0.29885849072268444)
            )
        )

    def test_minimum_distance_points_line(self):
        lineseg = edges.LineSegment3D(design3d.Point3D(0, 1.5, -1), design3d.Point3D(0, -0.3, 0.5))
        test_minimum_distance_points_line = self.arc3d.minimum_distance(lineseg, True)
        self.assertTrue(
            test_minimum_distance_points_line[1].is_close(
                design3d.Point3D(0.0, 0.17973768284826552, 0.10021859762644536)
            )
        )
        self.assertTrue(
            test_minimum_distance_points_line[2].is_close(
                design3d.Point3D(0.577350269233884, 0.5773502691674968, 0.5773502691674968)
            )
        )
    ...
    def test_extrusion(self):
        ...
        expected_points = [
            design3d.Point3D(0.5773502691896258, 0.5773502691896258, 0.5773502691896258),
            design3d.Point3D(0.9902758761944115, -0.09837095360773679, -0.09837095360773679),
            design3d.Point3D(0.3300336747301572, -0.6674869937100307, -0.6674869937100307),
            design3d.Point3D(-0.6878679668410815, -0.5132434413579569, -0.5132434413579569),
            design3d.Point3D(-0.9855985596534889, 0.2004326678135493, -0.43957897955165043),
            design3d.Point3D(-0.7535426476176112, 0.2422554725826881, -1.1719580897904072),
            design3d.Point3D(0.23911314674674883, 0.020511957781626844, -1.3937016045914683),
            design3d.Point3D(0.9726406135015547, 0.5428353031885098, -0.8713782591845853),
            design3d.Point3D(0.6521117099944177, 1.2431806183060983, -0.17103294406699687),
            design3d.Point3D(0.5773502691896258, 0.5773502691896258, 0.5773502691896258),
        ]
    ...
    def test_linesegment_intersections(self):
        lineseg1 = edges.LineSegment3D(design3d.Point3D(2, 2, 0), design3d.Point3D(-2, -2, 0))
        lineseg2 = edges.LineSegment3D(design3d.Point3D(0, -1, -1), design3d.Point3D(0, 1, 0))
        lineseg3 = edges.LineSegment3D(
            design3d.Point3D(-1, -1, -1), design3d.Point3D(-math.sqrt(2) / 2, -math.sqrt(2) / 2, 0)
        )
        ...
        expected_point = design3d.Point3D(-0.7071067811865476, -0.7071067811865475, 0.0)
        ...
        point = design3d.Point3D(0, 1.5, -1)
        lineseg = edges.LineSegment3D(point, point + (self.list_points[-5] - point) * 2)
        lineseg2 = edges.LineSegment3D(point, point + (design3d.Point3D(1, 1.5, -1) - point) * 2)
        ...
        self.assertEqual(
            arc3d_lineseg_inters[0], design3d.Point3D(-0.5773502691896256, -0.5773502691896257, -0.5773502691896257)
        )
    ...
    def test_arc_intersections(self):
        ...
        self.assertTrue(
            intersections[0].is_close(design3d.Point3D(-1.2905737351057338, -5.961765089245487, -0.9872550297484957))
        )
        self.assertTrue(
            intersections[1].is_close(design3d.Point3D(7.332605916292026, -4.272128323148586, -0.42404277438286175))
        )
    ...
    def test_minimum_distance_bspline(self):
        ...
        self.assertTrue(
            min_dist_point1.is_close(design3d.Point3D(-1.29057373513423, -5.961765089185557, -0.9872550297440593))
        )
        self.assertTrue(
            min_dist_point2.is_close(design3d.Point3D(-1.2905737351257018, -5.9617650891813065, -0.9872550297271026))
        )

    def test_move_frame_along(self):
        origin = self.arc3d.start
        w = self.arc3d.unit_direction_vector(0.0)
        u = self.arc3d.unit_normal_vector(0.0)
        if not u:
            u = w.deterministic_unit_normal_vector()
        v = w.cross(u)
        starting_frame = design3d.Frame3D(origin, u, v, w)
