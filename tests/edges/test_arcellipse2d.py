...
class TestArcEllipse2D(unittest.TestCase):
    ...
    def test_point_belongs(self):
        ...
        self.assertFalse(self.arc_ellipse2d.point_belongs(design3d.Point2D(1.4142135623730954, 1.4142135623730947)))
    ...
    def test_linesegment_intersections(self):
        lineseg = edges.LineSegment2D(design3d.Point2D(2, -1), design3d.Point2D(-2, 2))
        inters = self.arc_ellipse2d.linesegment_intersections(lineseg)
        self.assertEqual(len(inters), 2)
        self.assertTrue(inters[1].is_close(design3d.Point2D(-0.5154201866080642, 0.8865651399560482)))
        self.assertTrue(inters[0].is_close(design3d.Point2D(1.0636435368618713, -0.29773265264640336)))

    def test_translation(self):
        translated_ellipse = self.arc_ellipse2d.translation(design3d.Vector2D(1, 1))
        self.assertTrue(translated_ellipse.start.is_close(design3d.Point2D(1.5, 2.5)))
        self.assertTrue(translated_ellipse.end.is_close(design3d.Point2D(2.5, 1.5)))

    def test_point_distance(self):
        point = design3d.Point2D(0, 0)
    ...
    def test_point_at_abscissa(self):
        ...
        self.assertTrue(point.is_close(design3d.Point2D(0.2409700344646443, -0.10841585996396141)))
    ...
    def test_normal_vector(self):
        ...
        self.assertTrue(direc_vector.is_close(design3d.Vector2D(-1.1622013637446964, 0.25201219862839874)))

    def test_direction_vector(self):
        direc_vector = self.arc_ellipse2d.direction_vector(self.arc_ellipse2d.length() * 0.4)
        self.assertTrue(direc_vector.is_close(design3d.Vector2D(0.25201219862839874, 1.1622013637446964)))
