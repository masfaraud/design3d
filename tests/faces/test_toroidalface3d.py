...
class TestToroidalFace3D(unittest.TestCase):
    surface1 = surfaces.ToroidalSurface3D(design3d.OXYZ, 0.32, 0.08)
    ...
    def test_from_contours3d(self):
        ...
        surface = surfaces.ToroidalSurface3D.from_json(os.path.join(folder, "repair_periodicity_toroidal_surface.json"))
        contour = wires.Contour3D.from_json(os.path.join(folder, "repair_periodicity_toroidal_surface_contour.json"))
        face = faces.ToroidalFace3D.from_contours3d(surface, [contour])
        self.assertAlmostEqual(face.surface2d.area(), math.pi**2, 4)
        self.assertTrue(face.surface2d.outer_contour.is_ordered())

        surface = surfaces.ToroidalSurface3D.from_json(os.path.join(folder, "face_with_inner_contour_surface.json"))
        ...
        contour0 = wires.Contour3D.from_json(os.path.join(folder, "repair_inner_contour_periodicity_contour_0.json"))
        contour1 = wires.Contour3D.from_json(os.path.join(folder, "repair_inner_contour_periodicity_contour_1.json"))
        face = faces.ToroidalFace3D.from_contours3d(surface, [contour0, contour1])
    ...
    def test_planeface_intersections(self):
        ...
        ts = surfaces.ToroidalSurface3D(design3d.OXYZ, 2, 1)
        tf = faces.ToroidalFace3D.from_surface_rectangular_cut(ts, -1.4, 3.5, 0.0, 2.5)
        ...
        for i, n in enumerate(npy.linspace(0, math.pi / 4, 10)):
            ...
            self.assertTrue(
                all(
                    tf.point_belongs(p, 1e-4) and plane_face.point_belongs(p, 1e-4)
                    for i in planeface_intersections
                    for p in i.primitives[0].points
                )
            )
    ...
    def test_cylindricalface_intersections(self):
        expected_results = [
            ...
            [2.7679468571575105],
            [2.8109179729321183],
            [1.3806998569480715, 3.0283316710422508],
            [2.1248782869459646],
            [1.7368478889595058],
            [2.55833794579346],
            [2.8123613465408064, 1.3899450251554277],
            [2.447515630586587],
        ]
        ...
        for i, theta in enumerate(npy.linspace(0, math.pi * 0.7, 10)):
    ...
    def test_conicalface_intersections(self):
        tf = faces.ToroidalFace3D.from_surface_rectangular_cut(
            surfaces.ToroidalSurface3D(design3d.OXYZ, 2, 1), -1.4, 3.5, 0.0, 2.5
        )
        conical_surface = surfaces.ConicalSurface3D(design3d.OXYZ.translation(design3d.Vector3D(-1, 0, 0)), math.pi / 6)
    ...
    def test_triangulation_quality(self):
        ...
        for i1, i2, i3 in triangulation.triangles:
            ...
            self.assertLess(self.surface1.point_distance(triangle.middle()), self.surface1.minor_radius * 0.05)
    ...
    def test_normal_at_point(self):
        toroidalsurface = surfaces.ToroidalSurface3D(design3d.OXYZ, 2, 1)
        toroidaface = faces.ToroidalFace3D.from_surface_rectangular_cut(toroidalsurface, -1.4, 3.5, 0.0, 2.5)

        point = design3d.Point3D(1.1520373740641632, 2.008364391878863, 0.9489846193555862)

        normal = toroidaface.normal_at_point(point)
        self.assertTrue(normal.is_close(design3d.Vector3D(0.1568952782807088, 0.27351794069082946, 0.9489846193555862)))
