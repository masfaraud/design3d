...
import design3d
from design3d import edges, wires, faces, surfaces, curves
...
class TestPlaneFace3D(unittest.TestCase):
    face_with_3holes = SerializableObject.from_json(os.path.join(folder, "face_with_3holes.json"))
    face = SerializableObject.from_json(os.path.join(folder, "face_to_cut_the_one_with_3holes.json"))
    plane_face_cylindricalface_intersec = SerializableObject.from_json(
    ...
    def test_point_belongs(self):
        volume_model = SerializableObject.from_json(os.path.join(folder, "test_planeface_point_belongs.json"))
        self.assertTrue(volume_model.primitives[0].point_belongs(volume_model.primitives[1]))

    def test_plane_face_intersection(self):
        face1, face2 = SerializableObject.from_json(
            os.path.join(folder, "test_planeface_intersections.json")
        ).primitives
        ...
        face1, face2 = faces.PlaneFace3D.from_json(os.path.join(folder, "test_planef_inters291123.json")).primitives
        face_intersections = face1.face_intersections(face2)
        line_seg = edges.LineSegment3D(
            design3d.Point3D(0.034031786272172244, 0.019018077708099396, 0.07196336225667499),
            design3d.Point3D(0.03756212352555941, 0.022324611455208112, 0.07196336225667499),
        )
    ...
    def test_face_inside(self):
        face2 = self.face.frame_mapping(
            design3d.Frame3D(
                design3d.Point3D(0, 0, 0),
                design3d.Vector3D(0.5, 0, 0),
                design3d.Vector3D(0, 0.5, 0),
                design3d.Vector3D(0, 0, 0.5),
            ),
            "old",
        )
        self.assertEqual(self.face.face_inside(face2), True)
        self.assertEqual(face2.face_inside(self.face), False)
        face1, face2 = SerializableObject.from_json(os.path.join(folder, "test_face_inside.json")).primitives
        self.assertTrue(face1.face_inside(face2))
        face1, face2 = SerializableObject.from_json(os.path.join(folder, "test_face3_face_inside.json")).primitives
    ...
    def test_line_intersections(self):
        line_inside_hole = curves.Line3D(design3d.Point3D(0.1, 0.0, -0.3), design3d.Point3D(-0.1, 0.0, 0.3))
        line_inside_face = curves.Line3D(design3d.Point3D(-0.05, 0.0, -0.3), design3d.Point3D(-0.1, 0.0, 0.3))
    ...
    def test_divide_face(self):
        face_intersections = self.face.face_intersections(self.face_with_3holes)
        cutting_contours = self.face_with_3holes.get_face_cutting_contours({self.face_with_3holes: face_intersections})
        ...
        cutting_contour = wires.Wire2D.from_points(
            [
                design3d.Point2D(0.5, 1.0),
                design3d.Point2D(0.5, 0.75),
                design3d.Point2D(1, 0.75),
                design3d.Point2D(1, 1),
                design3d.Point2D(1.25, 1),
                design3d.Point2D(1.25, 1.5),
                design3d.Point2D(1, 1.5),
            ]
        )
        face_tobe_divided = SerializableObject.from_json(os.path.join(folder, "face_tobe_divided.json"))
        ...
        expected_faces_areas = [
            [0.005578804359415823, 0.0948396741089057, 0.23430978309161565, 0.0055788043593624215],
            [0.0855613934860544, 0.032085522557644186, 0.01069517418574345],
            [0.002005345159845676, 0.002005345159820638, 0.0033422419331328506, 0.0006684483866419249],
            [0.3403070659192998, 0.005578804359415823],
            [0.010695174185850198, 0.08556139348605463, 0.0427806967433878],
            [0.07754001284661505, 0.002673793546460905],
        ]
        file_names = [
            "test_face_divide_face5.json",
            "test_face_divide_face2.json",
            "test_planeface3d_divide_face.json",
            "test_face_divide_face3.json",
            "test_face_divide_face.json",
    ...
    def test_set_operations_new_faces(self):
        volumemodel = SerializableObject.from_json(os.path.join(folder, "test_set_operations_new_faces.json"))
    ...
    def test_cylindricalface_intersections(self):
        ...
        face = faces.CylindricalFace3D.from_surface_rectangular_cut(cylindricalsurface, 0, design3d.TWO_PI, -0.25, 0.25)
        """ ========== CIRCLE3D ========="""
        plane_face_3 = self.plane_face_cylindricalface_intersec.rotation(design3d.O3D, design3d.X3D, math.pi / 2)
        ...
        self.assertEqual(face_intersections[0].primitives[0].circle.center, design3d.O3D)
        self.assertEqual(face_intersections[0].primitives[0].circle.radius, 0.15)
        """ ========== FULL ELLIPSE3D ========="""
        plane_face_3 = self.plane_face_cylindricalface_intersec.rotation(design3d.O3D, design3d.X3D, math.pi / 4)
        face_intersections = plane_face_3.face_intersections(face)
        self.assertEqual(len(face_intersections), 1)
        self.assertIsInstance(face_intersections[0].primitives[0], edges.FullArcEllipse3D)
        self.assertEqual(face_intersections[0].primitives[0].ellipse.center, design3d.O3D)
        self.assertAlmostEqual(face_intersections[0].primitives[0].ellipse.major_axis, 0.21213203435596426)
        self.assertTrue(
            face_intersections[0]
            .primitives[0]
            .ellipse.major_dir.is_close(design3d.Vector3D(0, -0.7071067811865475, 0.7071067811865475))
        )
        """ ========== THREE ARC ELLIPSES ========="""
        plane_face_3 = self.plane_face_cylindricalface_intersec.rotation(design3d.O3D, design3d.X3D, math.pi / 7)
        face_intersections = plane_face_3.face_intersections(face)
        ...
        self.assertTrue(
            face_intersections[0]
            .primitives[0]
            .ellipse.major_dir.is_close(
                design3d.Point3D(2.6567716615652136e-17, -0.4338837391180807, 0.9009688679021675)
            )
        )
        self.assertAlmostEqual(face_intersections[0].primitives[0].ellipse.major_axis, 0.3457147306439571)
        list_expected_points = [
            [
                design3d.Point3D(0.08947272158306664, -0.12039365470206077, 0.25),
                design3d.Point3D(0.136637076048, -0.061889493851, 0.128514858204),
                design3d.Point3D(0.15, -0.0, 0.0),
            ],
            [
                design3d.Point3D(0.15, -0.0, 0.0),
                design3d.Point3D(0.136637075473, 0.06188949512, -0.128514860839),
                design3d.Point3D(0.08947272158306664, 0.12039365470206077, -0.25),
            ],
            [
                design3d.Point3D(-0.08947272158306664, 0.12039365470206077, -0.25),
                design3d.Point3D(-0.15, 0, 0),
                design3d.Point3D(-0.08947272158306664, -0.12039365470206077, 0.25),
            ],
        ]
        ...
        """ ========== TWO PARALLEL LINES ========="""
        plane_face_3 = self.plane_face_cylindricalface_intersec.rotation(design3d.O3D, design3d.X3D, math.pi)
        face_intersections = plane_face_3.face_intersections(face)
        self.assertTrue(
            face_intersections[0]
            .primitives[0]
            .is_close(edges.LineSegment3D(design3d.Point3D(0.15, 0.0, -0.25), design3d.Point3D(0.15, 0.0, 0.25)))
        )
        self.assertTrue(
        ...
        """ ========== ONE LINE ========="""
        plane_face_3 = self.plane_face_cylindricalface_intersec.translation(R * design3d.Y3D)
        face_intersections = plane_face_3.face_intersections(face)
        self.assertEqual(
            face_intersections[0].primitives[0],
            edges.LineSegment3D(design3d.Point3D(0.0, 0.15, -0.25), design3d.Point3D(0.0, 0.15, 0.25)),
        )
        """======= [] ==========="""
        planeface, cylface = SerializableObject.from_json(
            os.path.join(folder, "test_planeface_cylindricalface_intersections_none.json")
        ).primitives
    ...
    def test_conical_face_intersections(self):
        ...
        conical_face = faces.ConicalFace3D.from_surface_rectangular_cut(conical_surface, 0, 1.5 * math.pi, 0.0, 1)
        """======== Arc3D ========="""
        ...
        plane_origin = frame.origin - normal * 0.5
    ...
    def test_merges_faces(self):
        ...
        faces_areas = []
        file_names = [
            "test_merge_faces4.json",
            "test_merge_faces5.json",
            "faces_merge_faces2.json",
            "faces_merge_faces3.json",
            "faces_merge_faces4.json",
        ]
        for filename in file_names:
            file_path = os.path.join(source_folder, filename)
            obj = SerializableObject.from_json(file_path)
        ...
        expected_faces_areas = [
            ...
            [
                0.05578804359331602,
                0.005578804359362449,
                0.011157608718620371,
                0.022315217437398727,
                0.05020923923410867,
            ],
        ]
        for solution, expected_solution in zip(faces_areas, expected_faces_areas):
    ...
    def test_grid_points(self):
        surface3d = surfaces.Plane3D(design3d.OXYZ)
        outer_contour2d = wires.Contour2D.from_circle(curves.Circle2D.from_center_and_radius(design3d.O2D, 1.0))
        inner_contour2d = wires.Contour2D.from_circle(
            curves.Circle2D.from_center_and_radius(design3d.O2D, 0.25, is_trigo=False)
        )
