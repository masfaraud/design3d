...
class TestCylindricalFace3D(unittest.TestCase):
    ...
    cylindrical_face2 = faces.CylindricalFace3D.from_surface_rectangular_cut(cylindrical_surface2, 0, 3.14, 0.0, 8.0)

    def test_linesegment_intersections(self):
        lineseg3d = edges.LineSegment3D(design3d.O3D, design3d.Point3D(0.3, 0.3, 0.3))
        ...
        lineseg3d_2 = edges.LineSegment3D(design3d.Point3D(-0.3, -0.3, -0.1), design3d.Point3D(0.3, 0.3, 0.3))
    ...
    def test_triangulation_quality(self):
        ...
        for i1, i2, i3 in triangulation.triangles:
            ...
            # triangle.plot(ax=ax)
            # Test orthogonality
            self.assertAlmostEqual(triangle.surface3d.frame.w.dot(point1 - point2), 0.0)
            ...
            self.assertLess(
                self.cylindrical_surface2.point_distance(triangle.middle()), self.cylindrical_surface2.radius * 0.05
            )

    def test_from_contours3d(self):
        surface = surfaces.CylindricalSurface3D.from_json(os.path.join(folder, "surface_openned_one_contour.json"))
        contour3d_0 = wires.Contour3D.from_json(os.path.join(folder, "contour3d__openned_one_contour_0.json"))
        contour3d_1 = wires.Contour3D.from_json(os.path.join(folder, "contour3d__openned_one_contour_1.json"))
        ...
        contour3d_0 = wires.Contour3D.from_json(os.path.join(folder, "cylindrical_contour_0_repair_contour2d.json"))
        contour3d_1 = wires.Contour3D.from_json(os.path.join(folder, "cylindrical_contour_1_repair_contour2d.json"))
        ...
        surface = SerializableObject.from_json(os.path.join(folder, "surface3d_1.json"))
        contour0 = SerializableObject.from_json(os.path.join(folder, "contour_1_0.json"))
        contour1 = SerializableObject.from_json(os.path.join(folder, "contour_1_1.json"))
        ...
        surface = SerializableObject.from_json(os.path.join(folder, "cylindrical_surface_floating_point_error.json"))
        contour0 = SerializableObject.from_json(os.path.join(folder, "cylindrical_contour_floating_point_error.json"))
    ...
    def test_split_by_plane(self):
        ...
        cylindricalface = faces.CylindricalFace3D.from_surface_rectangular_cut(
            cylindricalsurface, 0, design3d.TWO_PI, -0.25, 0.25
        )
        ...
        list_expected_points = SerializableObject.from_json(
            os.path.join(folder, "test_cylindrical_faces_split_by_plane_" "expected_discretization_points.json")
        ).primitives
    ...
    def test_plane_intersections(self):
        face, plane = SerializableObject.from_json(
            os.path.join(folder, "test_buggy_split_by_plane12_07_2023.json")
        ).primitives
    ...
    def test_conicalface_intersections(self):
        expected_results = [
            ...
            [
                [0.904180630293272, 1.392773884071054],
                [2.754671034122705, 0.7935213452250598],
                [0.9945099178084125, 0.011885799104577068, 0.49133092691300395, 1.0377142752022748, 0.5464208749923458],
                [0.2895638627891746, 0.9393502379009631, 2.569944707187624],
            ...
            [
                ...
                [0.49133092691300395, 1.0377142752022748, 0.5464208749923458],
                [2.569944707187624],
                [],
            ],
        ]
        conical_surface = surfaces.ConicalSurface3D(design3d.OXYZ, math.pi / 6)
        conical_face = faces.ConicalFace3D.from_surface_rectangular_cut(conical_surface, 0, design3d.TWO_PI, 0, 2)

        cylindrical_surface1 = surfaces.CylindricalSurface3D(
            design3d.Frame3D(design3d.Point3D(0.3, 0.3, 0.8), design3d.Y3D, design3d.Z3D, design3d.X3D), 0.3
        )
        cylindrical_surface2 = surfaces.CylindricalSurface3D(
            design3d.Frame3D(design3d.Point3D(0, 0, 0.5), design3d.Y3D, design3d.Z3D, design3d.X3D), 0.3
        )
        cylindrical_surface3 = surfaces.CylindricalSurface3D(
            design3d.Frame3D(design3d.Point3D(0, 0, 1), design3d.Y3D, design3d.Z3D, design3d.X3D), 0.3
        )
        ...
        cylindrical_surface4 = surfaces.CylindricalSurface3D(
            design3d.Frame3D(center, design3d.Y3D, design3d.Z3D, design3d.X3D), math.tan(conical_surface.semi_angle) / 2
        )
    ...
    def test_face_inside(self):
        face1, face2 = SerializableObject.from_json(
            os.path.join(folder, "test_cylindricalface_face_inside.json")
        ).primitives
