...
def geometric_set(arguments, object_dict, *args, **kwargs):
    ...
    """
    # TODO: IS THIS RIGHT?
    primitives = [object_dict[int(node[1:])] for node in arguments[1]]
    return primitives
...
def faceted_brep_shape_representation(arguments, object_dict, *args, **kwargs):
    ...
    shells = []
    for arg in arguments[1]:
        if isinstance(object_dict[int(arg[1:])], d3dshells.Shell3D):
...
def shape_representation(arguments, object_dict, *args, **kwargs):
    ...
    for arg in arguments[1]:
        if (
        ...
        elif int(arg[1:]) in object_dict and isinstance(object_dict[int(arg[1:])], d3dshells.Shell3D):
            shells.append(object_dict[int(arg[1:])])
            ...
            if not all(component is None for component in [frame.u, frame.u, frame.w]):
                frames.append(frame)
        elif int(arg[1:]) in object_dict and isinstance(object_dict[int(arg[1:])], design3d.edges.Arc3D):
            shells.append(object_dict[int(arg[1:])])
        elif int(arg[1:]) in object_dict and isinstance(object_dict[int(arg[1:])], design3d.edges.BSplineCurve3D):
            shells.append(object_dict[int(arg[1:])])
        else:
            pass
...
def frame_map_closed_shell(closed_shells, item_defined_transformation_frames, shape_representation_frames):
    ...
    for shell3d in closed_shells:
        ...
        new_frame = design3d.Frame3D(
            ...
            design3d.Vector3D(*transfer_matrix[0]),
            design3d.Vector3D(*transfer_matrix[1]),
            design3d.Vector3D(*transfer_matrix[2]),
        )
    ...
    if arguments[2] in object_dict:
        if isinstance(object_dict[arguments[2]], list):  # arguments = {, , [], [], item_....}
            if (
                ...
                and not isinstance(object_dict[arguments[2]][0], design3d.Frame3D)
            ...
            if (
                object_dict[arguments[2]]
                and isinstance(object_dict[arguments[2]][0], design3d.Frame3D)
                and not isinstance(object_dict[arguments[3]][0], design3d.Frame3D)
            ):
                return frame_map_closed_shell(
