...
def product_writer(current_id, product_name):
    ...
    step_content += f"#{product_id} = PRODUCT('{product_name}'," f"'{product_name}','',(#{product_context_id}));\n"
    product_definition_formation_id = product_id + 1
    step_content += f"#{product_definition_formation_id} = " f"PRODUCT_DEFINITION_FORMATION('','',#{product_id});\n"
    ...
    step_content += f"#{product_definition_shape_id} = PRODUCT_DEFINITION_SHAPE(''," f"'',#{product_definition_id});\n"
    shape_definition_repr_id = product_definition_shape_id + 1
    shape_representation_id = shape_definition_repr_id + 1
    step_content += (
        f"#{shape_definition_repr_id} = SHAPE_DEFINITION_REPRESENTATION("
...
def geometric_context_writer(current_id, uncertainty: float = 1e-4):
    ...
    step_content += (
        f"#{uncertainty_id} = UNCERTAINTY_MEASURE_WITH_UNIT(LENGTH_MEASURE({uncertainty}),"
        f"#{legth_unit_id},'distance_accuracy_value','confusion accuracy');\n"
    )
    step_content += (
        f"#{geometric_representation_context_id} = ( GEOMETRIC_REPRESENTATION_CONTEXT(3) "
...
def assembly_definition_writer(current_id, assembly_data, component_data, assembly_frame_id, component_frame_id):
    ...
    step_content += (
        f"#{context_dependent_id} = CONTEXT_DEPENDENT_SHAPE_REPRESENTATION("
        f"#{repr_relationship_with_transfo_id},#{product_definition_shape_id});\n"
    )
    step_content += (
        f"#{repr_relationship_with_transfo_id} = ( REPRESENTATION_RELATIONSHIP('','',"
    ...
    step_content += (
        f"#{item_defined_transfo_id} = ITEM_DEFINED_TRANSFORMATION('','',#{assembly_frame_id},"
        f"#{component_frame_id});\n"
    )
    step_content += (
        f"#{product_definition_shape_id} = PRODUCT_DEFINITION_SHAPE('Placement',"
        f"'Placement of an item',#{next_assembly_usage_occurrence_id});\n"
    )
    step_content += (
        f"#{next_assembly_usage_occurrence_id} = NEXT_ASSEMBLY_USAGE_OCCURRENCE("
